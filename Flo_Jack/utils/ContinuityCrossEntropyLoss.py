...
class ContinuityCrossEntropyLoss:
    ...
    def compute_continuity_loss(self, outputs, labels, mask):
        ...
        for n in range(len(outputs)): #for every timeseries
            ...
            fw_output_continuity = smax_output_continuity[1:] - smax_output_continuity[:-1]
            fw_output_continuity = torch.abs(fw_output_continuity)
            fw_output_continuity = torch.sum(fw_output_continuity) / 2
            ...
            continuity_loss[n] = torch.abs(output_continuity-labels_continuity)
